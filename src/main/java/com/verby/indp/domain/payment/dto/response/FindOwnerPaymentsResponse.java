...
public record FindOwnerPaymentsResponse(
    List<PaymentItem> payments,
    int totalPages,
    long totalElements
) {

    public static FindOwnerPaymentsResponse from(Page<StoreSubscription> page) {
        List<PaymentItem> items = page.getContent().stream()
            .map(PaymentItem::from)
            .toList();
        return new FindOwnerPaymentsResponse(items, page.getTotalPages(), page.getTotalElements());
    }
